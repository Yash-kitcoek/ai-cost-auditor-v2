...
function ScoreBadge({ score }: { score: number }) {
  ...
  const label =
    score >= 80 ? 'Well Optimized' : score >= 50 ? 'Room to Improve' : 'Significant Overspend';
  return (
    <div
      style={{
        display: 'flex',
  ...
  if (result.isAlreadyOptimal) {
    return (
      <div style={{ ...card, textAlign: 'center', padding: '36px 24px' }}>
        ...
        <h2
          style={{ fontSize: 26, fontWeight: 800, margin: '0 0 8px', color: '#10b981' }}
        >
  ...
  return (
    <div
      style={{
        ...card,
        background:
          'linear-gradient(135deg, rgba(99,102,241,0.12) 0%, rgba(168,85,247,0.08) 50%, rgba(16,185,129,0.08) 100%)',
...
function CredexCTA({ monthlySavings }: { monthlySavings: number }) {
  ...
  return (
    ...
    >
      ...
      <h3
        style={{ fontSize: 18, fontWeight: 800, margin: '0 0 8px', color: 'white' }}
      >
      ...
      >
        ...
        Enterprise, and more — sourced from companies that overforecast. Your audit
        shows{' '}
...
function BenchmarkBlock({
  ...
  const benchmarks: Record<
    string,
    Record<string, { median: number; p25: number; p75: number; label: string }>
  > = {
    solo: {
      coding:   { median: 35,  p25: 20, p75: 60,  label: 'solo devs' },
      writing:  { median: 25,  p25: 0,  p75: 40,  label: 'solo writers' },
      data:     { median: 30,  p25: 15, p75: 55,  label: 'solo analysts' },
      research: { median: 20,  p25: 0,  p75: 40,  label: 'solo researchers' },
      mixed:    { median: 30,  p25: 15, p75: 55,  label: 'solo professionals' },
    },
    small: {
      coding:   { median: 55,  p25: 30, p75: 90,  label: '2–5 person teams' },
      writing:  { median: 30,  p25: 10, p75: 50,  label: '2–5 person teams' },
      data:     { median: 45,  p25: 20, p75: 80,  label: '2–5 person teams' },
      research: { median: 35,  p25: 15, p75: 60,  label: '2–5 person teams' },
      mixed:    { median: 45,  p25: 20, p75: 75,  label: '2–5 person teams' },
    },
    mid: {
      coding:   { median: 75,  p25: 40, p75: 130, label: '6–20 person teams' },
      writing:  { median: 35,  p25: 15, p75: 60,  label: '6–20 person teams' },
      data:     { median: 60,  p25: 30, p75: 110, label: '6–20 person teams' },
      research: { median: 45,  p25: 20, p75: 80,  label: '6–20 person teams' },
      mixed:    { median: 60,  p25: 30, p75: 100, label: '6–20 person teams' },
    },
    large: {
      coding:   { median: 90,  p25: 50, p75: 160, label: '21–100 person orgs' },
      writing:  { median: 40,  p25: 20, p75: 70,  label: '21–100 person orgs' },
      data:     { median: 75,  p25: 40, p75: 130, label: '21–100 person orgs' },
      research: { median: 55,  p25: 25, p75: 95,  label: '21–100 person orgs' },
      mixed:    { median: 70,  p25: 35, p75: 120, label: '21–100 person orgs' },
    },
    enterprise: {
      coding:   { median: 120, p25: 70, p75: 200, label: '100+ person orgs' },
      writing:  { median: 50,  p25: 25, p75: 90,  label: '100+ person orgs' },
      data:     { median: 100, p25: 55, p75: 170, label: '100+ person orgs' },
      research: { median: 70,  p25: 35, p75: 120, label: '100+ person orgs' },
      mixed:    { median: 95,  p25: 50, p75: 160, label: '100+ person orgs' },
    },
  };
  ...
  const verdictColor: string =
    isLean || isNormal ? '#10b981' : isHigh ? '#f59e0b' : '#ef4444';
      ...
      >
        <div>
          <div style={{ fontSize: 11, color: '#555', marginBottom: 4 }}>
            Your spend / developer
          </div>
          <div style={{ fontSize: 36, fontWeight: 900, color: verdictColor }}>
            ${yourPerDev}
            <span style={{ fontSize: 14, fontWeight: 400, color: '#555' }}>/mo</span>
          </div>
          <div style={{ fontSize: 11, fontWeight: 600, color: verdictColor, marginTop: 2 }}>
            {verdict}
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ fontSize: 12, color: '#666', marginBottom: 2 }}>
            25th pct: <strong style={{ color: '#888' }}>${b.p25}/dev</strong>
          </div>
          <div style={{ fontSize: 12, color: '#666', marginBottom: 2 }}>
            Median: <strong style={{ color: '#888' }}>${b.median}/dev</strong>
          </div>
          <div style={{ fontSize: 12, color: '#666' }}>
            75th pct: <strong style={{ color: '#888' }}>${b.p75}/dev</strong>
          </div>
        </div>
      </div>
...
function ShareBlock({
  ...
  return (
    <>
      ...
      >
        Your email, company name, and role are excluded from this link — only tool
        usage, costs, and savings are shown. Safe to share with your team or CFO.
      </p>
      <div
        style={{
          display: 'flex',
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            border: `1px solid ${
              copied ? 'rgba(16,185,129,0.3)' : 'rgba(99,102,241,0.3)'
            }`,
...
function PDFExportButton({
  ...
  async function handleExport() {
    setLoading(true);
    try {
      const jspdfModule = await import('jspdf').catch(() => {
        throw new Error('Run: npm install jspdf');
      });
      ...
      addRect(0, 0, 210, 297, [10, 10, 15]);
      addRect(margin, y, contentW, 1, [99, 102, 241]);
      y += 6;
      addText('AI SPEND AUDIT REPORT', margin, y, { size: 8, color: [99, 102, 241] });
      y += 6;
      addText('AI Cost Audit', margin, y, {
        size: 20,
        style: 'bold',
        color: [255, 255, 255],
      });
      y += 10;
      addText(
        `Audit ID: ${auditId.slice(0, 8)} · ${new Date().toLocaleDateString()}`,
        margin,
        y,
        { size: 8, color: [100, 100, 120] }
      );
      y += 12;
      ...
      if (!result.isAlreadyOptimal && result.totalMonthlySavings > 0) {
        ...
        addText(formatCurrency(result.totalMonthlySavings), margin + 6, y + 20, {
          size: 18,
          style: 'bold',
          color: [99, 102, 241],
        });
        addText('ANNUALLY', margin + 90, y + 7, { size: 7, color: [150, 150, 200] });
        addText(formatCurrency(result.totalAnnualSavings), margin + 90, y + 20, {
          size: 18,
          style: 'bold',
          color: [168, 85, 247],
        });
        y += 34;
        addText(
          `Current: ${formatCurrency(result.totalCurrentSpend)}/mo  →  Optimised: ${formatCurrency(result.totalOptimizedSpend)}/mo`,
          margin,
          y,
          { size: 9, color: [120, 120, 140] }
        );
        y += 8;
      } else {
        addRect(margin, y, contentW, 14, [17, 24, 17]);
        addText(
          'Stack is already well-optimised — no significant overspend found.',
          margin + 6,
          y + 9,
          { size: 9, color: [16, 185, 129] }
        );
        y += 20;
      }
      ...
      const scoreColor: [number, number, number] =
        result.score >= 80
          ...
          ? [245, 158, 11]
          : [239, 68, 68];
      addText(`Efficiency score: ${result.score}/100`, margin, y, {
        size: 9,
        style: 'bold',
        color: scoreColor,
      });
      ...
      if (result.summary) {
        checkPage(30);
        addText('AI ANALYSIS', margin, y, { size: 7, color: [99, 102, 241] });
        y += 5;
        addRect(margin, y, contentW, 0.5, [40, 40, 60]);
        y += 4;
        y =
          addText(result.summary, margin, y, {
            size: 9,
            color: [180, 180, 200],
            maxWidth: contentW,
          }) + 8;
      ...
      checkPage(20);
      addText('RECOMMENDATIONS', margin, y, { size: 7, color: [99, 102, 241] });
      y += 5;
      addRect(margin, y, contentW, 0.5, [40, 40, 60]);
      y += 6;

      const ac: Record<string, [number, number, number]> = {
        downgrade: [245, 158, 11],
        switch:    [99, 102, 241],
        cancel:    [239, 68, 68],
        keep:      [16, 185, 129],
      };
      ...
      for (const rec of result.recommendations) {
        ...
        const aColor: [number, number, number] = ac[rec.action] ?? [120, 120, 140];
        doc.setFillColor(...aColor);
        doc.rect(margin + 3, y + 6, 16, 9, 'F');
        addText(actionLabel(rec.action).toUpperCase(), margin + 11, y + 12, {
          size: 6,
          style: 'bold',
          color: [10, 10, 15],
          align: 'center',
        });
        addText(rec.toolName, margin + 23, y + 9, {
          size: 10,
          style: 'bold',
          color: [240, 240, 255],
        });
        addText(
          ...
          margin + 23,
          y + 16,
          { size: 8, color: [100, 100, 130] }
        );
        if (rec.savings > 0) {
          addText(`-${formatCurrency(rec.savings)}/mo`, pageW - margin - 3, y + 13, {
            size: 10,
            style: 'bold',
            color: [16, 185, 129],
            align: 'right',
          ...
          addText('Optimal', pageW - margin - 3, y + 13, {
            size: 9,
            color: [16, 185, 129],
            align: 'right',
          });
      ...
      y += 6;
      addText(
        'Powered by AI Spend Audit · aicostaudit.com · Pricing verified May 2026',
        pageW / 2,
        y,
        { size: 7, color: [60, 60, 80], align: 'center' }
      );
  ...
  async function validateCode() {
    ...
    try {
      const res = await fetch(
        `/api/referral?code=${encodeURIComponent(inputCode.trim().toUpperCase())}`
      );
      const d = await res.json();
  ...
  return (
    <div style={card}>
      ...
      <p style={{ fontSize: 12, color: '#555', margin: '0 0 14px', lineHeight: 1.6 }}>
        Share your code with a teammate or founder friend. When they book a Credex
        consultation,{' '}
        <strong style={{ color: '#a5b4fc' }}>both of you</strong> get 1 month of
        Credex credits free.
      </p>
      {!code ? (
        <button
      ...
      <div
        style={{ borderTop: '1px solid rgba(255,255,255,0.06)', margin: '14px 0' }}
      />
      <div style={{ fontSize: 11, color: '#555', marginBottom: 6 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <input
          ...
          style={{ ...fieldStyle, flex: 1, fontFamily: 'monospace', letterSpacing: 2 }}
        />
        <button
      ...
      {codeError && (
        <div style={{ fontSize: 11, color: '#fca5a5', marginTop: 6 }}>{codeError}</div>
      )}
      {perk && (
        <div
          style={{
        ...
        >
          ✓ Code applied! Perk unlocked: <strong>{perk}</strong> — mention this code
          when you book your Credex consultation.
        </div>
      )}
    </div>
  );
...
export default function ResultPage() {
  ...
  if (notFound) {
    return (
      ...
      >
        <div
          style={{ textAlign: 'center', maxWidth: 360, padding: '0 16px' }}
        >
  ...
  return (
    ...
    >
      <div
        style={{ maxWidth: 680, margin: '0 auto', padding: '48px 16px 80px' }}
      >
        ...
        {optimizedRecs.length > 0 && (
          <div style={card}>
            ...
            >
              ⚡ {optimizedRecs.length} Immediate Action
              {optimizedRecs.length > 1 ? 's' : ''} to Reduce Spend
            </div>
        ...
        <div style={card}>
          ...
          <p style={{ fontSize: 12, color: '#555', margin: '0 0 12px' }}>
            Download a formatted PDF — useful for sharing with your CFO or attaching
            to a budget review.
          </p>
        ...
        <div style={card}>
          ...
          >
            Drop this snippet into any blog post — your readers get a live audit tool
        ...
        >
          Pricing data current as of May 2026 · Built with ♥ for founders who hate
          waste
        </div>
      </div>
    </div>
